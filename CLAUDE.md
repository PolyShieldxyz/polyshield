...
packages/
  ...
  backend/       Node.js (signing-layer, proof-relay, indexer, mock-clob-server, mock-env)
                 ...
                 VaultEventIndex → /recovery-data + /events (SQLite merkle.db). Clients never re-scan chain.
                 FC-15: proof-relay ALSO hosts the public market CATALOG (marketCatalog.ts: Gamma sync →
                 /markets, /markets/search via Gamma public-search, /markets/prices) + anonymous aggregate
                 analytics (analytics.ts, POST /analytics — no wallet/IP/id). Markets page reads this.
...
- **Operator-driven settlement:** When a market resolves, the Signing Layer calls `Vault.resolveMarket(market_id)` (NO payout argument). The Vault **derives** the payout on-chain by reading the real Gnosis CTF and stores `pendingCredit[circuit_key][outcome_side]`. **CTF ABI (FC-12, mandatory):** read payouts via the ELEMENT accessor `ctf.payoutNumerators(conditionId, index)` + `ctf.getOutcomeSlotCount(conditionId)` — the `payoutNumerators(conditionId) → uint256[]` array getter exists ONLY on `MockCTF` and reverts on mainnet CTF. `ICTF.sol`, `Vault.resolveMarket`, and `VaultLogic.naCancellationCredit` all use the element accessor. `resolveMarket` runs BEFORE the collateral redemption (resolve-first) so a redeem failure never blocks settlement. Users' settlement proofs don't supply `payout_per_share`/`shares_held` — the Vault injects them.
- **RPC requirement (FC-12, operational):** production needs a **full/archive node with a usable `eth_getLogs` range**. Two RPCs were ruled out: pruned public nodes (publicnode — "History has been pruned") and **Alchemy free tier (caps `eth_getLogs` at a 10-block range** → ~10k requests for the history scan + blows the monthly compute budget). For short free-tier testing set `LOG_SCAN_CHUNK=10`. All services use `RetryingJsonRpcProvider` (single 429-retry layer) + windowed, cursor-persisted, de-nested log scans. See `docs/architecture.md` §2.5.
- **Backend index/cache is authoritative-read, never authoritative-truth (FC-12):** the proof-relay mirrors public on-chain state (`CachedMerkleTree` → `/merkle-path`; `VaultEventIndex` → `/recovery-data` + `/events`) so clients never re-scan the chain. The on-chain tree stays the source of truth. PRIVACY INVARIANT preserved: the backend serves only PUBLIC, anonymous data and never sees a secret — it cannot de-anonymize (only `Deposited` is wallet-keyed) and cannot forge notes (the client's recovery replay only acts on events matching its own derived nullifier). Do not move secret-dependent matching server-side.
...
- **Fee model — IMPLEMENTED (FC-10, refined by FC-14).** All rates live in one governance-mutable packed struct `Vault.feeConfig` (`struct FeeConfig { uint16 betFeeBps; uint64 relayGasFeeUSDC; uint64 minBet; uint64 withdrawalFeeUSDC; uint64 minWithdrawal; address feeRecipient; }`), set atomically via `setFeeParams(FeeConfig)` (onlyOwner). All fees accumulate in `Vault.feeAccumulator` (USDC in the pool) and are claimed by `feeRecipient` via `withdrawFees(amount)`. Owner-controlled initially; transferable to governance in P4. **Current live rates** (set via `setFeeParams`, 2026-06-20): `betFeeBps = 30` (0.3%), `relayGasFeeUSDC = $0.15` (1.5e5), `minBet = $1` (1e6), `withdrawalFeeUSDC = $1` (1e6), `minWithdrawal = $5` (5e6), `feeRecipient = owner`. (`initialize` seeds genesis defaults — `betFeeBps = 20` / `relayGasFeeUSDC = 0` / `minBet = $1` / `withdrawalFeeUSDC = $0.10` / `minWithdrawal = $1`, per FC-14 — but does NOT re-run on the deployed proxy; the live rate is whatever the last `setFeeParams` set.) Three fee types:
  - **Bet authorization fee + relay gas (in BET_AUTH circuit):** the Vault computes `fee = bet_amount * betFeeBps / 10000 + relayGasFeeUSDC` from `feeConfig` and **injects it as a public input** to `bet_auth` (10 public signals). The circuit enforces `new_balance = current_balance - bet_amount - fee`. Because the Vault — not the user — supplies `fee`, a forged proof with any other fee produces a `new_commitment` that fails verification (same anti-forgery pattern as the injected `bet_amount` for cancellations). Applies uniformly to every order type. The gas reimbursement is charged in USDC from the note (privacy-preserving), NOT a native-POL transfer (which would re-link wallet↔bet). `authorizeBet` reverts `BelowMinimum` if `bet_amount < minBet`. **FC-14: the fee is split into `protocolFee` (the bps part) and `relayFee` (the flat part); the sum is what's injected (unchanged), but `protocolFee` is stored in `mapping(bytes32 => uint64) betProtocolFee` and BOTH are emitted in `BetAuthorized`** — so recovery reconstructs the exact post-bet balance across rate changes, and refunds can be pro-rated. **Provisional vs claimable (`feeAccumulator` holds only CLAIMABLE fees):** the `relayFee` is added to `feeAccumulator` at `authorizeBet` (earned at submission); the `protocolFee` stays **provisional** in `betProtocolFee` and is **released into `feeAccumulator` only at a terminal-executed state** — `creditSettlement`, a full `closePosition`, or `naCancellationCredit` (via `_releaseProtocolFee`). So a refund NEVER decrements `feeAccumulator` (no underflow — the recipient can never withdraw a still-refundable fee). **Proportional refunds:** `betCancellationCredit` (bet never executed) refunds `bet_amount + protocolFee` and clears `betProtocolFee` (feeAccumulator untouched); `partialFillCredit` refunds `(bet_amount − spent) + ⌊protocolFee·(bet−spent)/bet⌋` and releases the executed-part fee to `feeAccumulator`; both **keep `relayFee`**. **N/A (void) markets keep the protocol fee** (the bet executed) → released as earned. No circuit/verifier change (only the Vault-injected refund *value* changed); user-facing refunds are identical so frontend/recovery are unaffected.
  - **Partial-fill UX (FC-14):** every market buy short-fills vs the fee-naive committed `expected_shares` (the Polymarket taker fee comes from the stake), so it lands `ACTIVE` with a `PARTIAL` attestation. `expected_shares` is kept as the committed **cryptographic cap** (settlement/close credit only down from it — bounds rogue-operator over-crediting); the frontend **auto-finalizes** silently (`finalizePartial.ts` builds the `partial_credit` proof with the in-memory V2 seed — no prompt — and relays it) to normalize the record to `FILLED`, so a normal fill shows **"Filled — N shares"** with no $0 "reclaim." Do NOT move payout authority to the operator (the cap is a pool-safety guardrail).
...
**EIP-170 / `VaultInputs.sol` + `VaultLogic.sol`:** the Vault is kept under the 24576-byte runtime limit by DELEGATECALL-linking **two** external libraries (after the FC-11-era refactor the Vault is ~20,165 B, ~4.4 KB headroom). Foundry auto-deploys + links both in tests and `forge script`.
- **`src/VaultInputs.sol`** holds (a) the Vault domain types at **file scope** — `BetStatus`, `BetRecord`, `OperatorAttestation`, `FeeConfig`, and the 8 public-input structs (`BetAuthPublicInputs`, …) — reference them as bare names, NOT `Vault.X`; and (b) `library VaultInputs`, each circuit's public-signal assembly + the `IVerifier.verify` dispatch (one `verify<Proof>(verifier, proof, inputs, injected)` per proof type, all `pure`/`view`). When adding a proof type or changing public inputs, edit `VaultInputs`.
- **`src/VaultLogic.sol`** (`library VaultLogic`) holds the bulkier spend-path function BODIES: `creditSettlement`, `betCancellationCredit`, `naCancellationCredit`, `closePosition`, `partialFillCredit`, `consolidate`, plus the shared `_checkAttestation`/`_requireUnspentKnownRoot`/`_spendAndInsert` helpers. Each runs under DELEGATECALL in the Vault's storage/context, so external calls (tree/nullifiers/ctf) carry `msg.sender == Vault` and emitted events log with the Vault's address. Contract handles are passed by value via a `Ctx` struct (`tree, nullifiers, operator, domainSeparator`); the Vault's own mappings (`betRecords`, `pendingCredit`, `marketResolvedAt`, `conditionIdOf`) are passed as `storage` references. Errors/events are re-declared in both (same selectors/topics; the Vault keeps its copies for ABI). The Vault keeps only thin wrappers (carrying `nonReentrant`/`whenNotPaused`) + a `_ctx()` builder. To add or grow a spend-path function, put the body in `VaultLogic`, not the Vault.

Before adding Vault bytecode, run `forge build --sizes`; if tight, move more logic into `VaultLogic`.
...
- **P3+ (wallet-derived secrets):** Call `getNoteSecret(sign, wallet, depositIndex, version)` (`secretSession.ts`) — V2 derives from the in-memory master seed (one signature unlocks it for the session), V1 signs the per-index message. New deposits use V2 (`getNoteSecret(..., 2)`) and tag the note `derivationVersion: 2`. Do NOT ask the user to back anything up — their wallet is their backup. Never use `crypto.getRandomValues()` for note secrets in P3+. (`deriveSecret`/`deriveSecretV1`/`deriveSecretV2`/`deriveMasterSeed` are the pure primitives in `notes.ts`.)
- **The note cache is persisted ENCRYPTED in IndexedDB** (FC-13, `cacheStore.ts`) — `(kind, balance, nonce, commitment, nullifier, spent, owner_address, createdAt, txHash, marketId, expectedShares, derivationVersion)` — encrypted with a non-extractable AES-GCM key. The in-memory cache is the synchronous working set, hydrated once on app start (`useNotesHydration`). The secret is NOT persisted anywhere; the master seed is held in memory only (never persisted). On a cache miss the sync getters return empty until hydration completes, so note-reading screens are gated on hydration.
- **P1/P2 recovery:** User must import their encrypted backup file and decrypt it with their wallet.
- **P3+ recovery:** Call `recoverNotes(wallet)` (or `recoverNotesViaBackend`) — one master-seed signature; it maps each `Deposited(W,...)` commitment to its index via the free V2 derivation (V1 per-index as fallback for legacy notes) and replays on-chain events. Silent reconcile auto-syncs new on-chain notes when the seed is already unlocked; a determinate progress bar drives the manual Restore/Sync.
- The deposit index counter `(wallet_address → count)`, `last_block`, and `chain_fp` stay in localStorage (non-sensitive; deposit count is already public via `/recovery-data`). The index counter is recoverable by scanning chain events if lost.
...
- Any change to the EIP-191 secret derivation message strings or version numbers (both the V1 `"PolyShield deposit derivation…Version: 1"` and the V2 `"PolyShield master seed…Version: 2"` constants are frozen — FC-13)
