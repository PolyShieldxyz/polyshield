        ...
        hours.map((_, hi) => {
          ...
          return (
            <rect key={`${di}-${hi}`} x={x} y={y} width={11} height={8} rx={1}
              fill={`oklch(0.82 0.13 210 / ${v * 0.6 + 0.05})`} />
          )
        })
      )}
      {days.map((day, di) => (
...
function ClusteringGraph() {
  ...
  return (
    <svg viewBox="0 0 280 140" width="100%">
      {nodes.map((n, i) => (
        <g key={i}>
          {n.you && <circle cx={n.x} cy={n.y} r={n.r + 6} fill="oklch(0.82 0.13 210 / 0.1)" />}
...
export default function PrivacyPage() {
  const metrics = [
    { label: 'Anonymity score', value: '94/100', color: 'var(--cyan)', note: 'Based on set size, timing entropy, decoy density' },
          ...
          <div>
            ...
            <div className="panel mt-4" style={{ padding: 20 }}>
              ...
              <div className="small mt-2" style={{ fontSize: 11 }}>
                No deposit-to-withdrawal cluster detected. Your activity pattern is indistinguishable from 312 other wallets.
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="panel mt-4" style={{ padding: 20 }}>
              ...
              <div className="col mt-3 gap-3">
                {[
                  ['Merkle anonymity', 'Your commitment is one of 48,225 leaves. Root membership proves inclusion without revealing which.', 'var(--cyan)'],
