...
- Circuit `consolidate.circom` + snarkjs Groth16 verifier `ConsolidateVerifier.sol` (slot 8), registered in `packages/circuits/pipeline/src/constants.ts`/`interfaces.ts` (with a multi-leaf fixture builder in `generateTestProofs.ts`). An `ONLY_CIRCUIT` env filter was added to the pipeline CLIs so a single circuit can be (re)built in isolation without re-keying the others.
...
### Implementation
- **Circuit** `groth16/bet_auth.circom`: +1 public input `fee` (range-checked), `new_balance = current_balance - bet_amount - fee`. Public signals 9→10. Verifier + zkey/wasm regenerated; `packages/circuits/pipeline/src/constants.ts` `bet_auth: 9→10`; `RealVerifier.t.sol` asserts 10 inputs.
